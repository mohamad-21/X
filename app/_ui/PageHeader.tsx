...
function PageHeader({ title, desc, endContent, children, sticky = true }: { title?: string, endContent?: React.ReactNode, desc?: string, children?: React.ReactNode, sticky?: boolean }) {
  const router = useRouter();

  return (
    <header className={`${sticky ? 'sticky top-0 left-0' : ''} w-full bg-background/40 backdrop-blur-lg px-2 z-[3]`}>
