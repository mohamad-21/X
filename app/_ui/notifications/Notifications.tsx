...
function Notifications({
  ...
  return (
    <div
      className="first:border-t border-b border-default p-4 bg-transparent hover:bg-default/15 transition cursor-pointer flex gap-2 to-twitt"
      onClick={handleNotifClick}
    >
      <div>
        {notif.type === "follow" && (
          <BiSolidUser size={30} className="text-primary" />
        )}
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="to-twitt">
          <h3 className="flex items-center gap-1">
            ...
            {notif.type === "like" && (
              <span className="to-twitt">{t("likedYourPost")}</span>
            )}
            {notif.type === "follow" && (
              <span className="to-twitt">{t("followedYou")}</span>
            )}
            {notif.type === "reply" && (
              <span className="to-twitt">{t("repliedToYourPost")}:</span>
            )}
          </h3>
        </div>
        {notif.type === "like" && (
          <div>
            {twitt ? (
              <TwittView twitt={twitt} user={user} />
            ) : (
